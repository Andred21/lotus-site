...
| `See More`              | `Cursos`            | `href="#"` — um único link com o texto `See More`, ao final da grade; os três links dos cards têm texto vazio e também apontam para `#` (`sections[].links` de `#Cursos` em `dom.json`) |
