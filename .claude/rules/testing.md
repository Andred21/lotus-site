...
- Gates atuais: `pnpm check` (`agent:check` + `format:check` + `lint` + `typecheck` + `test` + `build`). `pnpm e2e` fica fora de `check` porque precisa subir servidor; roda separado e é exigido no CI.
