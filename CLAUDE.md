...
- `tsconfig.json` é um solution file que referencia dois projetos buildados juntos por `tsc -b`: `tsconfig.app.json` (código de aplicação em `src/`) e `tsconfig.node.json` (código de build, ex. `vite.config.ts`).
